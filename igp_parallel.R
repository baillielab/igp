#clusterRepro modified source code
#clusterRepro_parallel function changed to allow parallel running of permutations
#permuteCol and IGP.clusterRepro_parallel unchanged
...
clusterRepro_parallel <- function (Centroids, New.data, Number.of.permutations,cl) 
{
  Result <- c()
  ...
  Calculation <- IGP.clusterRepro_parallel(New.data, Centroids)
  ...
  clusterExport(cl, list("Centroids.prime","SVD","Centroids","New.data"),envir=environment())
  
  IGP <- c()
  Size <- c()
  #parallelise process
  igp_parallel_res <- parLapply(cl,1:Number.of.permutations,
                                function(i){
                                  ...
                                  Calculation <- IGP.clusterRepro_parallel(New.data, New.centroids)
                                  IGP <- Calculation$IGP
                                  Size <- Calculation$Size
                                  return(list("IGP"=IGP,"Size"=Size))
                                }
                      )

  #extract values of interest
  IGP <-unlist(lapply(igp_parallel_res,function(x){x$IGP}))
  Size <- unlist(lapply(igp_parallel_res,function(x){x$Size}))
  
  Result$p.value <- c(NA, length(Actual.Size))
  Result$Number <- c(NA, length(Actual.Size))
  for (i in 1:length(Actual.Size)) {
    if (length(which(Size == Actual.Size[i])) == 0 || Actual.Size[i] == 
        0) {
      Result$p.value[i] <- NA
      Result$Number[i] <- 0
    }
...
function (Data, Centroids) 
{
  ...
  Data <- Data[Features, ]
  Centroids <- Centroids[Features, ]
  Correlation <- cor(Data, Centroids, use = "pairwise.complete.obs", 
                     method = "pearson")
  ...
  for (i in 1:nrow(Correlation)) {
    Classification[Class[i], Class[which.max(Correlation[i, 
                                                         ])]] <- Classification[Class[i], Class[which.max(Correlation[i, 
                                                                                                                      ])]] + 1
  }
  Result$Class <- Class
